...
const ChatView: React.FC<ChatViewProps> = ({ 
  ...
  return (
    <div className="w-full h-full flex flex-col p-4 md:p-8 max-w-6xl mx-auto relative" onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={e => { e.preventDefault(); setIsDragging(false); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f); }}>
      ...
      <footer className="relative flex items-center gap-3 p-2 rounded-full shadow-[0_15px_35px_rgba(0,0,0,0.4)] transition-all mb-4 mx-2 group" style={glassStyles}>
        ...
        <input type="text" placeholder={`Ngobrol sama ${config.name}...`} className="flex-1 bg-transparent outline-none py-3.5 text-sm font-semibold text-white placeholder:text-white/20" value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} />
        <button onClick={() => handleSend()} disabled={(!inputText.trim() && !attachedImage) || isTyping} className="p-3.5 bg-gradient-to-br from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 rounded-full transition-all active:scale-95 disabled:opacity-20 shadow-lg shadow-pink-500/20 border border-white/10 flex items-center justify-center mr-0.5"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white translate-x-0.5" viewBox="0 0 20 20" fill="currentColor"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" /></svg></button>
      </footer>
    </div>
  );
};
